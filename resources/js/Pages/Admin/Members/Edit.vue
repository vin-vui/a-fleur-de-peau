...
export default {
    ...
    methods: {
        memberUpdate(member) {
            ...
            this.form.post(route('members.update', member.id), {
        ...
        previewImg(e) {
            ...
            if (file) {
                ...
                this.previewImage = this.member.profile_picture;
            }
        },
    },
    data() {
        return {
            ...
            previewImage: this.member.profile_picture
        };
    }
}
</script>
...
<template>
    <AppLayout title="Editer un membre">
        <template #header>
        ...
        <div class="min-h-screen bg-white py-1 sm:py-12">
            <div class="relative sm:max-w-xl sm:mx-auto bg-white shadow-lg sm:rounded-3xl sm:p-20">
                ...
                <form @submit.prevent="memberUpdate(this.member)" enctype="multipart/form-data">
                    <div class="mb-4">
                        <label class="block text-[#00353F] mb-2" for="name">Nom</label>
                        <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                            type="text" name="name" id="name" v-model="form.name">
                    </div>

                    <div class="mb-4">
                        <label class="block text-[#00353F] mb-2" for="role">Rôle</label>
                        <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                            type="text" name="role" id="role" v-model="form.role">
                    </div>

                    <div class="mb-4">
                        <label class="block text-[#00353F] mb-2" for="description">Description</label>
                        <textarea class="shadow min-h-0 appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                            name="description" id="description" v-model="form.description"></textarea>
                    </div>
                    ...
                    <div class="flex justify-center">
                        <button class="shadow bg-[#00353F] hover:bg-white text-white hover:text-[#00353F] font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
                            type="submit">
                            Modifier
                        </button>
                    </div>
